...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="the supplement pixels parted by \",\""
    )
    parser.add_argument(
...
def location_supplement(src_loc, img_size, supplement):
    dst_loc = ()

    lower_x, upper_x = src_loc[0], src_loc[1]
    lower_y, upper_y = src_loc[2], src_loc[3]
    width = img_size[0]
    height = img_size[1]
    lower_left, upper_left = supplement[0], supplement[1]
    lower_right, upper_right = supplement[2], supplement[3]
    lower_top, upper_top = supplement[4], supplement[5]
    lower_bottom, upper_bottom = supplement[6], supplement[7]

    # supplement random size
...
def crop_all_imgs(all_imgs, all_labels):
    ...
    for img_idx, img_path in enumerate(all_imgs):
        ...
        for roi_idx, one in enumerate(label["text_lines"]):
            ...
            loc = location_supplement(
                    [lower_x, upper_x, lower_y, upper_y], 
                    [src_img.shape[1], src_img.shape[0]], 
                    )
            dst_img = croped_img(src_img, loc[0], loc[1], loc[2], loc[3])
            if dst_img is None:
                continue
...
if __name__ == '__main__':
    ...
    supplement = [int(args.supplement.split(",")[i]) for i in range(8)]
    ...
    imgpath_list, label_list = crop_all_imgs(all_imgs, all_labels, supplement)
