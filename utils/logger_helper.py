...
def init_logger(logdir, logger_name):
	...
	if logger_name not in Logger.manager.loggerDict:
		...
		handler = TimedRotatingFileHandler(os.path.join(logdir, "info.log"), when='midnight',backupCount=7)
		handler.setFormatter(formatter)
		handler.setLevel(logging.INFO)
		logger.addHandler(handler)
		
		# file handler of level error
		handler = TimedRotatingFileHandler(os.path.join(logdir, 'error.log'), when='midnight',backupCount=7)
		handler.setFormatter(formatter)
